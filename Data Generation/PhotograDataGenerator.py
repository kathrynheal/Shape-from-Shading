...
patchx,patchy = np.meshgrid(subset,subset+200) #each is patchsz x patchsz
...
gdZ = gaussD(Z,5,1) ##gaussian surface derivatives
L = alldata['L']
...
#ntlim=axs[0].imshow(NtL,origin='lower',cmap='gray')
#axs[1].title.set_text('NtL (Image w/o albedo)')
#fig.colorbar(ntlim, ax=axs[1], shrink=0.5)
...
imi = axs[0,1].imshow(Na,origin='lower',vmin=-3,vmax=3)
axs[0,1].title.set_text('Ying dfx')
fig.colorbar(imi, ax=axs[0,1], shrink=0.5)
imi = axs[1,0].imshow(gdZ[1],origin='lower',vmin=-3,vmax=3)
axs[1,0].title.set_text('my dfy')
fig.colorbar(imi, ax=axs[1,0], shrink=0.5)
imi = axs[1,1].imshow(Nb,origin='lower',vmin=-3,vmax=3)
axs[1,1].title.set_text('Ying dfy')
fig.colorbar(imi, ax=axs[1,1], shrink=0.5)
#plt.show()
###


gdNtL = gaussD(NtL,6,1)

fig = plt.figure()
tIyy = gdNtL[1]**2*gdNtL[5] - 2*gdNtL[4]*gdNtL[1]*gdNtL[2] + gdNtL[2]**2*gdNtL[3]
imIyy = plt.imshow(tIyy,origin='lower',vmin=-10,vmax=10)#-.000001,vmax=.000001)
